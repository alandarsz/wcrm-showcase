  ...
  <div class="min-h-screen flex items-center justify-center">
    <div class="w-full max-w-md bg-white rounded-xl p-8">
      <div class="flex flex-col items-center mb-6">
        ...
        <p class="text-sm text-gray-500 mt-1 text-center">Masukkan data akun untuk reset password</p>
      </div>
      <!-- from data -->
      <el-form ref="formRef" :model="form" :rules="rules" label-position="top" class="space-y-4" @submit.prevent="onSubmit">
        ...
        <el-button type="success" class="w-full" size="large" :loading="loading" native-type="submit" :disabled="!isFormValid">
          Verifikasi Data
        </el-button>
      </el-form>
      <!-- verifikasi akun -->
      <el-dialog v-model="verified" title="Verifikasi Akun" :width="dialogWidth()" :close-on-click-modal="false" :close-on-press-escape="false">
        ...
        <template #footer>
          ...
          <el-button type="primary" @click="onConfirmReset"> Reset Password </el-button>
        </template>
      </el-dialog>
      <!-- konfirmasi berhasil -->
      <el-dialog v-model="resetDoneDialog" title="Reset Password Selesai" :width="dialogWidth()" :close-on-click-modal="false" :close-on-press-escape="false">
        <div class="space-y-3 text-sm text-gray-600">
          <p>Reset password berhasil dilakukan.</p>
      ...
      <div class="my-6 text-center text-sm">
        <router-link to="/login" class="text-emerald-600 hover:underline"> ← Kembali ke Login </router-link>
      </div>
    </div>
...
const resetDoneDialog = ref(false);
