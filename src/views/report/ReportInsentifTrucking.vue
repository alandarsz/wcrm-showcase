  ...
  <div class="bg-[#ffffff] flex flex-col items-center">
    <div class="w-full p-2">
      <div class="flex items-center justify-between mb-3">
        <h1 class="text-lg font-semibold">Report Bonus Trucking</h1>
        <!-- filter -->
      </div>
      ...
      <div>
        <div>
          ...
          >
            ...
            <el-table-column prop="sales" label="Nama Sales" min-width="250" sortable/>
            <el-table-column prop="TanggalInvoiceDibuat" label="Tanggal Invoice Dibuat" min-width="180" sortable />
            <el-table-column prop="NominalInvoice" label="Nominal Ongkir" min-width="120" sortable>
              <template #default="{ row }"> {{ formatRupiah(row.NominalInvoice) }}</template>
            </el-table-column>
            <el-table-column prop="PPH" label="PPH" min-width="80" sortable>
              <template #default="{ row }"> {{ formatRupiah(row.PPH) }}</template>
            </el-table-column>
            <el-table-column label="Insentif(%)" min-width="120" sortable>
              <template #default="{ row }"> {{ row.persen }}% </template>
            </el-table-column>
            <el-table-column prop="NominalInsentif" label="Insentif" min-width="120" sortable>
...
function doPaginate(index = 1, pSize) {
  getListData(API.getDataInsentifTrucking, index, pSize || pageSize.value, search.value, filterData.value);
}
...
const getSummaries = ({ columns, data }) => {
  ...
  const numericColumns = ['nominal', 'pph', 'totalInsentif', 'total_insentif_hasil'];
