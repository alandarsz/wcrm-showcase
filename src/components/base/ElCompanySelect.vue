<template>
  <el-select-v2
    ...
    :options="listData"
    @focus="(!modelValue && listData.length < 5) || listData.length <= 1 ? queryCompanyList('') : ''"
...
import { queryCompanyList, companyList as listData, companyListLoading } from '../../helper/autoCompleteQuery';
...
function setByID() {
  ...
  if (!isFound) {
    queryCompanyList(modelValue.value.toString(), true);
  }
}
